...
export var min_size: Vector2 = Vector2(0, 0)

## Maximum size that the parent node will be
export var max_size: Vector2 = Vector2(0, 0)
...
func clamp_v2(v, minn, maxx):
	v.x = clamp(v.x, minn.x, maxx.x)
	v.y = clamp(v.y, minn.y, maxx.y)
	return v
